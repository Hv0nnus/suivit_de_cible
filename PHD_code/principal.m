...
ps = 0.98; # disparition une fois qu'il est né 
beta = 0.2; # Naissance
lambda = 0.000001; # nombre de false alarm
pd = 0.95; # est ce qu'on voit bien le truc a bon endroit
...
for k=1:T-1
  [x_kalm{k+1} P_kalm{k+1} weight{k+1}, n_k] = filtre_de_kalman( F, Q, H, R, vecteur_y{k+1},x_kalm{k}, P_kalm{k},n_k,weight{k},ps,pd,lambda,nombre_gaussien_a_garder,threshold,k);
end
...
for t=1:T
  ...
  drawnow;
  hold on
  %print(f2,strcat("Reel_Gif",int2str(t),".png"))
  %pause(0.1);
end
