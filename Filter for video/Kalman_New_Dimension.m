...
function [x_kalm_mean,x_kalm] = Kalman_New_Dimension(M,H,T,F,MQ,Q,R,x_init,vecteur_y_disparity,variance_initial,number_sampling,f_d, b,dPP)
  ...
  x_kalm_disparity(1:3,:,1) = U; % put of start value to the vector that will store the position
  ...
  for k=1:T-1
    samples_disparity = sampling_in_disparity(x_kalm_disparity(:,:,k), P_kalm_disparity,number_sampling,M);
