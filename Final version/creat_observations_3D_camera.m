function [vecteur_y] = creat_observations_3D_camera(H,R_camera,vecteur_x,T,f_d,b)

  vecteur_x_right = real_to_right_camera(vecteur_x,f_d,b);
  vecteur_x_left = real_to_left_camera(vecteur_x,f_d,b);

  vecteur_x_right(1:2,:) = vecteur_x_right(1:2,:) + sqrtm(R_camera) * randn(2,T);
  vecteur_x_left(1:2,:) = vecteur_x_left(1:2,:) + sqrtm(R_camera) * randn(2,T);
